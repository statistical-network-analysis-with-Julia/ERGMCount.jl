...
- Allows non-zero lower bounds, **negative ones included**: with `a < 0` a
  dyad's count may be negative — the estimator enumerates `a:b`, the Gibbs
  sampler draws from it (an edge is stored for every non-zero count, its
  `:weight` the negative value), `nonzero` counts `y ≠ 0`, and `gof`'s
  count-value panel runs over the negative values too
- Useful when zero is not a valid edge value (`a > 0`: an absent edge is then
  refused with a hint to widen the reference)

**When to use**:
- Edge values must be at least some minimum (e.g., rating scales starting at 1)
- Data has known lower and upper bounds, possibly signed (net balances, ±k
  ratings)
...
| Dyad law with a `sum` coefficient $\theta$ | Poisson$(\lambda e^\theta)$ | Geometric, success probability $1 - e^\theta$ ($\theta < 0$) | Binomial$(n, \text{logistic}(\theta))$ | tilted uniform |
...
Each reference measure can draw from its baseline distribution — the law of
a dyad when every coefficient is 0 (for the parameter-free counting measure
and the binomial, a conventional Geometric(0.5) and Binomial(n, 0.5)). The
Gibbs sampler does **not** use these draws: it redraws each dyad from its
full conditional under the fitted coefficients, in which the reference enters
through `log_reference`. `sample_reference` is a convenience for seeding or
